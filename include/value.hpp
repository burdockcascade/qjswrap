...
namespace qjswrap {
    namespace detail { 
        ...
        struct JSClassInfo { 
            ...
            static inline std::unordered_map<JSRuntime*, JSValue> prototypes; // Track prototypes per runtime for secure instantiation
    ...
    public: 
        ...
        static JSValue create_js_value(JSContext* context, T&& v) { 
            ...
            if constexpr (std::is_same_v<Decayed, bool>) { 
                ...
                // Thread-safe lookup of both the Class ID and prototype registry mapping
                { 
                    ...
                    if (detail::JSClassInfo<Decayed>::prototypes.contains(rt)) {
                        proto = JS_DupValue(context, detail::JSClassInfo<Decayed>::prototypes.at(rt));
                    }
                } 
                
                if (class_id != 0) { 
                    // FIX: Link the instance object with its registered prototype methods
                    JSValue obj = JS_NewObjectProtoClass(context, proto, class_id); 
        ...
        Value& set_variable(const std::string_view name, T&& v) { 
            ...
            JS_SetPropertyStr(ctx, val, name_cstr.c_str(), create_js_value(ctx, std::forward<T>(v))); 
            return *this; 
        } 
        ...
        Value& set_constant(const std::string_view name, T&& v) { 
            std::string name_cstr(name);
            JS_DefinePropertyValueStr(ctx, val, name_cstr.c_str(), create_js_value(ctx, std::forward<T>(v)), JS_PROP_ENUMERABLE); 
            return *this; 
        } 
        ...
        [[nodiscard]] std::optional<T> as() const { 
            ...
            if constexpr (std::is_same_v<Decayed, Value>) { 
                ...
                if (const char* cstr = JS_ToCStringLen(ctx, &len, val)) { 
                    // FIX: Capturing the C-string pointer inside a reference-counted handle using custom deleter 
                    // to prevent memory leaks while preventing a dangling reference pointer inside the returned string_view.
                    auto shared_cstr = std::shared_ptr<const char>(cstr, [context = this->ctx](const char* p) {
                        if (p) JS_FreeCString(context, p);
                    });
                    return std::string_view(shared_cstr.get(), len); 
                } else { 
                    JS_FreeValue(ctx, JS_GetException(ctx));
                } 
            } else if constexpr (std::is_class_v<Decayed>) { 
